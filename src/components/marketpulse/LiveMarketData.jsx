...
export function useMarketData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await base44.functions.invoke('liveMarketContext', {});
      const payload = response?.data;
      if (!payload?.ok) throw new Error(payload?.error || 'Market context is unavailable.');
      setData(payload.data || {});
      setLastUpdated(payload.fetched_at ? new Date(payload.fetched_at) : new Date());
    } catch (err) {
...
export function MarketTile({ name, value, change, direction, subtext, sparkData, closed }) {
  ...
  useEffect(() => {
    if (previousValue.current !== value && previousValue.current != null) {
  ...
  const flashClass = flash === 'up' ? 'bg-emerald-400/10' : flash === 'down' ? 'bg-red-400/10' : '';
  return (
    <div className={`glass rounded-xl p-4 hover:border-primary/20 transition-all duration-200 group ${flashClass} ${closed ? 'opacity-60' : ''}`}>
...
export function RefreshBar({ lastUpdated, loading, onRefresh }) {
  return (
    <div className="flex items-center justify-between text-xs text-muted-foreground mb-6">
      <span>
        {lastUpdated
          ? `Market data · Updated ${lastUpdated.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}`
          : 'Fetching market data…'}
      </span>
      <button onClick={onRefresh} disabled={loading} className="flex items-center gap-1.5 hover:text-foreground transition-colors disabled:opacity-50">
